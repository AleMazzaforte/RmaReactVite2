...
export const Magia: React.FC = () => {
    ...
    const [productosAgregadosFactura, setProductosAgregadosFactura] = useState<any[]>([]);
    const [mostrarListaFactura, setMostrarListaFactura] = useState(false);
    ...
    const limpiarInputsProducto = () => {
        ...
        const cantidadInput = document.getElementById("cantidadStock") as HTMLInputElement;
        cantidadInput.value = "";
    };
    ...
    const agregarProducto = () => {
        ...
        if (!productoSeleccionado) {
            const skuInput = document.getElementById("skuInputStock") as HTMLInputElement;
            sweetAlert.fire({
                icon: "warning",
                title: "Campo vacío",
        ...
        if (!numeroPresupuesto || numeroPresupuesto.trim() === "") {
            const presupuestoInput = document.getElementById("numeroPresupuesto") as HTMLInputElement;
            sweetAlert.fire({
                icon: "warning",
                title: "Campo vacío",
    ...
    const limpiarInputsProductoFactura = () => {
        ...
        const cantidadInput = document.getElementById("cantidadFactura") as HTMLInputElement;
        cantidadInput.value = "";
    };

    const agregarProductoFactura = () => {
        setMostrarListaFactura(true);

        if (!numeroFactura || numeroFactura.trim() === "") {
            const facturaInput = document.getElementById("numeroFactura") as HTMLInputElement;
            sweetAlert.fire({
                icon: "warning",
                title: "Campo vacío",
        ...
        if (!productoSeleccionadoFactura) {
            const skuInput = document.getElementById("skuInputFactura") as HTMLInputElement;
            sweetAlert.fire({
                icon: "warning",
                title: "Campo vacío",
                text: "Debe seleccionar un producto",
        ...
        const producto = {
            ...
            sku: productoSeleccionadoFactura.sku,
            cantidad: cantidad,
            numFactura: numeroFactura,
        };

        setProductosAgregadosFactura([...productosAgregadosFactura, producto]);
        limpiarInputsProductoFactura();
    ...
    const limpiarInputsFactura = () => {
        setProductoSeleccionadoFactura(null);
        setMostrarListaFactura(false);
        setNumeroFactura("");
        const form = document.getElementById("formFactura") as HTMLFormElement;
        form.reset();
    };

    const eliminarProductoFactura = (index: number) => {
        const nuevosProductos = productosAgregadosFactura.filter((_, i) => i !== index);
        setProductosAgregadosFactura(nuevosProductos);

        if (productosAgregadosFactura.length == 1) {
            setMostrarListaFactura(false);
            setProductosAgregadosFactura([]);
        }
    };

    const enviarFormularioFactura = async (e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
        ...
        if (productosAgregadosFactura.length === 0) {
            sweetAlert.fire({
                icon: "warning",
                title: "Campo vacío",
                text: "Debe agregar al menos un producto",
        ...
        const formData = {
            numFactura: numeroFactura,
            productos: productosAgregadosFactura.map((producto) => ({
                modelo: producto.modelo,
                cantidad: producto.cantidad,
            })),
        };
        ...
        try {
            ...
            if (response.ok) {
                ...
                sweetAlert.fire({
                    icon: "error",
                    title: "Error",
                    text: "Hubo un problema al agregar la factura",
                });
            }
        } catch (error) {
            console.error("Error al enviar el formulario:", error);
    ...
    return (
        <div className="absolute justify-end w-full h-full p-4">
                    ...
                    <form id="formFactura" className="space-y-6">
                        ...
                        <div>
                            ...
                            <ListarProductos
                                ...
                                value={productoSeleccionadoFactura ? productoSeleccionadoFactura.sku : ""}
                            />
                        </div>
            ...
            {mostrarListaFactura && (
                <div className="bg-white absolute bottom-4 right-0 pb-2">
                    <h3 className="text-xl font-semibold mb-4">Factura: {numeroFactura}</h3>
                    <table>
                        <thead>
                            <tr>
                                <th>SKU</th>
                        ...
                        <tbody>
                            {productosAgregadosFactura.map((producto, index) => (
                                <tr
                                    key={index}
                                    className={index % 2 === 0 ? "bg-gray-200" : "bg-gray-50"}
                                >
                            ...
                            <button
                                type="button"
                                onClick={enviarFormularioFactura}
                                className="w-full py-1 px-2 bg-blue-500 mt-5  text-white  rounded-lg hover:bg-blue-700 focus:outline-black focus:ring focus:ring-blue-300"
                            >
                                {loadingFactura ? "Cargando..." : "Guardar Factura"}
                            </button>
                        </tbody>
                    </table>
                </div>

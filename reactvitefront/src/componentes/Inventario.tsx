...
useEffect(() => {
  ...
  const agregarAReposicion = (): void => { // Especificamos el tipo de retorno
  ...
  const exportarAExcel = (productosParaExportar: Producto[]) => {
    ...
    if (mostrarNoAsignados) nombreArchivo += "_NoAsignados";
  ...
  const extractSkuCantidad = (data: any[]): Map<string, number> => {
    ...
    data.forEach((row) => {
      const sku = row["SKU"] || row["sku"] || row["Código"] || row["Código depósito"];
          ...
          <div style={{
            ...
            {skuSeleccionadoReposicion && (
              <div style={{
                ...
                <span>SKU seleccionado: <strong>{skuSeleccionadoReposicion}</strong></span>
                <button

  ...
  if (productosSistema.length > 0 && resultados.length > 0) {
    ...
    const resultadosActualizados = resultados.map(categoria => {
      const itemsActualizados = categoria.items.map(item => {
        ...
        const total = (productoEnSistema.cantSistemaFemex || 0) + 
    ...
    console.log("Resultados actualizados:", resultadosActualizados); // Debug
    setResultadosConSistema(resultadosActualizados);
  }
}, [resultados, productosSistema]);
useEffect(() => {
  console.log("Productos sistema:", productosSistema);
  console.log("Resultados con sistema:", resultadosConSistema);
}, [productosSistema, resultadosConSistema]);
      ...
      <div className="w-full max-w-6xl">
        {resultadosConSistema.length > 0 ? (
          <table className="border border-gray-300 w-full table-auto">
            <thead>
              <tr className="bg-blue-300">
                ...
                <th className="p-2 text-center min-w-[120px]">Total sistema</th>
                <th className="p-2 text-center min-w-[120px]">Total contado</th>
                <th className="p-2 text-center min-w-[120px]">Diferencia</th>
              </tr>
            </thead>
            <tbody>
                ...
                <React.Fragment key={`cat-${catIndex}`}>
                  ...
                  {categoria.items.map((item, itemIndex) => (
                    <tr 
                      ...
                      className={itemIndex % 2 === 0 ? "bg-white" : "bg-blue-50"}
                    >
          ...
          <div className="text-center py-4 text-gray-500">
            {loading ? 'Cargando datos del sistema...' : 'Complete el formulario y haga click en Mostrar totales'}
          </div>
        )}
      </div>
    </div>

...
const clienteController = {
  getListarClientesRma: async (req, res) => { console.log('listarcliente')
    try {
...
const cargarRma = {
  getUltimoRemito: async (req, res) => {
    let connection;
    try {
        // Obtener la conexión a la base de datos (dependiendo de la librería que uses)
        connection = await conn.getConnection(); // Asegúrate de tener una función que obtenga la conexión a la base de datos

        // Consulta SQL para obtener el último número de ingreso
        const query = 'SELECT nIngreso FROM r_m_a ORDER BY nIngreso DESC LIMIT 1';
        ...
        if (result.length > 0) {
            const ultimoNIngreso = result[0].nIngreso;
            res.status(200).json({ ultimoNIngreso });
        } else {
            // Si no hay registros, devolver 0 o un valor inicial
            res.status(200).json({ ultimoNIngreso: 0 });
        }
    } catch (error) {
